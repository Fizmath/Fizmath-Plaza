...
func (s server) EditProduct(ctx context.Context, request *storespb.EDPRequest) (*storespb.EDPResponse, error) {

	err := s.app.EditProduct(ctx, commands.AddProduct{
		ID:          request.GetId(),
	...
	return &storespb.EDPResponse{}, err
}
...
func (s server) RemoveST(ctx context.Context, request *storespb.RemoveSTRequest) (*storespb.RemoveSTResponse, error) {

	err := s.app.RemoveST(ctx, commands.RemoveST{
		ID: request.GetId(),
	})

	return &storespb.RemoveSTResponse{}, err
}

func (s server) RemoveAllProds(ctx context.Context, request *storespb.RPRequest) (*storespb.RPResponse, error) {
	err := s.app.RemoveO(ctx, commands.RemoveO{
		ID: request.GetId(),
	})

	return &storespb.RPResponse{}, err
}

func (s server) RemoveIds(ctx context.Context, request *storespb.RemoveIdsRequest) (*storespb.RemoveIdsResponse, error) {

	fmt.Println(request.Ids, "server.go")
	items := make([]*commands.Idss, 0, len(request.Ids))
	for _, item := range request.Ids {
		items = append(items, s.itemToDomain(item))
	}

	err := s.app.RemoveIDS(ctx, commands.RemoveProduct{
		Ids: items,
	})

	return &storespb.RemoveIdsResponse{}, err
}

func (s server) itemToDomain(item *storespb.Ids) *commands.Idss {
...
func (s server) GetAllProds(ctx context.Context, request *storespb.GetAllRequest) (*storespb.GetAllResponse, error) {
	products, err := s.app.GetAllProduct(ctx)
	if err != nil {
		return nil, err
	}
	...
	return &storespb.GetAllResponse{
		Products: protoProducts,
	}, nil
}
