...
func InjectStoreRepository(tableName string, db *sql.DB) StoreRepository {
...
func (r StoreRepository) Find(ctx context.Context, storeID string) (*domain.Store, error) {
	const query = "SELECT name, location, participating FROM %s WHERE id = $1 "
...
func (r StoreRepository) Delete(ctx context.Context, storeID string) error {
	...
	fmt.Println("store deleted after all products dogs ...")
