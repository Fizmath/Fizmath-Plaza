...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
  CREATE DATABASE fizmathplaza;

  CREATE USER fizmathplaza_user WITH ENCRYPTED PASSWORD 'fizmathplaza_pass';

  GRANT CONNECT ON DATABASE fizmathplaza TO fizmathplaza_user;
EOSQL
