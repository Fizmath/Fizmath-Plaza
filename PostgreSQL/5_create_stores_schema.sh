...
psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "fizmathplaza" <<-EOSQL
  ...
  GRANT USAGE ON SCHEMA stores TO fizmathplaza_user;
  GRANT USAGE, SELECT ON ALL SEQUENCES IN SCHEMA stores TO fizmathplaza_user;


  GRANT INSERT, UPDATE, DELETE, SELECT ON ALL TABLES IN SCHEMA stores TO fizmathplaza_user;
EOSQL
